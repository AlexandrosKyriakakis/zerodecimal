...
func FuzzWideAggregates(f *testing.F) {
	...
	f.Add(aggregateFuzzEncode(maxValue, maxValue, Zero, maxValue.Neg()), uint8(0), uint8(TowardZero))
	f.Add(aggregateFuzzEncode(maxValue, maxValue, maxValue.Neg(), maxValue.Neg(), MustNew(1, -2)), uint8(2), uint8(ToNearestEven))
